...
class NegativeAPITestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.student2_token = login.json().get('access')
    ...
    def test_create_comment_as_non_member(self):
        ...
        # Student pertama mendaftar ke kursus
        self.client.post(
            f'{self.base_url}courses/{self.course.id}/enroll/',
            **{'HTTP_AUTHORIZATION': f'Bearer {self.student_token}'}
        )

        # Student kedua (belum terdaftar) mencoba membuat komentar
        response = self.client.post(
